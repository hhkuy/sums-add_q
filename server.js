...
app.get('/api/test', (req, res) => {
  res.json({
    message: "Server up, reordering and edit questions feature active",
...
app.get('/api/get-subtopic-file', async (req, res) => {
  ...
  try {
    ...
    const dec = Buffer.from(info.content, 'base64').toString('utf-8');
    let arr;
    try { arr = JSON.parse(dec); } catch (e) { arr = []; }
...
// 7) Delete subtopic file from repo
